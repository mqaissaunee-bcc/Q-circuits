...
sheet draws, so a new part gets an icon for free and an edited symbol keeps
its button in step. `PART_ICONS` in `src/parts.js` overrides that for the
few parts the rule serves badly: chips whose full symbol (a 74154 is 340
